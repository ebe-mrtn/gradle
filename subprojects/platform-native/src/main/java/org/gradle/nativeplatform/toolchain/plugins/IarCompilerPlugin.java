...
import org.gradle.nativeplatform.toolchain.internal.iar.IarToolChain;
...
@Incubating
@NonNullApi
public class IarCompilerPlugin implements Plugin<Project> {
    ...
    static class Rules extends RuleSource {
        ...
        public static void addToolChain(NativeToolChainRegistryInternal toolChainRegistry, ServiceRegistry serviceRegistry) {
            ...
            toolChainRegistry.registerFactory(Iar.class, new NamedDomainObjectFactory<Iar>() {
            	@Override
                public Iar create(String name) {
                    return instantiator.newInstance(IarToolChain.class, instantiator, name, buildOperationExecutor, OperatingSystem.current(), fileResolver, execActionFactory, compilerOutputFileNamingSchemeFactory, metaDataProviderFactory, workerLeaseService);
                }
            });
            toolChainRegistry.registerDefaultToolChain(IarToolChain.DEFAULT_NAME, Iar.class);
        }
